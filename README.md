...
3. 找到下载的文件（通常在"下载"文件夹里），右键 → "全部解压缩"（Mac双击就行）
4. 打开 Trae，点左上角"文件" → "打开文件夹"，选中解压出来的那个文件夹，点"选择"

海外用户用这个地址：`https://github.com/adsorgcn/iLang-Trae-Plugin/archive/refs/heads/main.zip`

### 方式二：用Trae终端下载（推荐，以后更新方便）

1. 打开 Trae
2. 按 **Ctrl+J**（Mac按 **Cmd+J**），屏幕下方会弹出一个黑色的框，这叫"终端"
3. 复制下面这行，粘贴进去，按回车：
...
4. 等几秒下载完，点"文件" → "打开文件夹"，找到 **iLang-Trae-Plugin** 文件夹打开

海外用户把 `gitee.com/palmmedia` 换成 `github.com/adsorgcn`。

**如果提示"git不是可识别的命令"或"git: command not found"：**

在 Trae 的对话框里直接跟AI说："帮我安装git"。AI会给你一行安装命令，复制粘贴到终端执行就行。装完之后重新试上面的第3步。
